...
export class ZardSelectComponent implements ControlValueAccessor, AfterContentInit, OnDestroy {
  ...
  onDropdownKeydown(e: Event): void {
  ...
  selectItem(value: string, label: string): string | void {
  ...
  private navigateItems(direction: number, items: HTMLElement[]): void {
    if (items.length === 0) {
      return;
    }
  ...
  private selectFocusedItem(items: HTMLElement[]): void {
    const currentIndex = this.focusedIndex();
  ...
  private focusFirstItem(items: HTMLElement[]): void {
    if (items.length > 0) {
  ...
  private focusLastItem(items: HTMLElement[]): void {
    if (items.length > 0) {
  ...
  private updateItemFocus(items: HTMLElement[], focusedIndex: number): void {
